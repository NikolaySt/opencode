...
OpenCode has an exclusive `"memory"` plugin slot for persistent cross-session recall. Only one memory plugin can be active at a time.
...
Memory plugins typically use the `agent.start` hook to inject retrieved context and `tool.result.persist` to index new information. [Learn more](/docs/memory#extend-with-plugins) about how memory works and how to build a memory plugin.
