...
[Learn more](/docs/memory#work-with-compaction) about how compaction and memory work together.

---

### Plugins

You can configure plugin slots and per-plugin settings through the `plugins` option. The `memory` slot is exclusive — only one memory plugin can be active.
...
- `slots` - Exclusive slot assignments (e.g. `{ memory: "plugin-id" }`)
- `entries` - Per-plugin enable/disable and configuration

[Learn more](/docs/memory#extend-with-plugins) about memory plugins and [available plugins](/docs/ecosystem#plugins).
